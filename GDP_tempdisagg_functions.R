# Functions for use in temporal disaggregation of GDP I&E based
# October 5, 2022; December 13, 2022

#------------------------------------------------------------------------------
# Function to retrieve monthly data, compare the end month to lastmonF, and if
# the end month retrieved is earlier than lastmonF, extend the time series 
# using an ARIMA model
RetrieveMon <- function(vnumber,firstmon,lastmonF) {
  y <- get_cansim_vector(vnumber,firstmon)
  if (y$Date[nrow(y)]==lastmonF) return(y)
  if (y$Date[nrow(y)]>lastmonF) return(y <- filter(y,Date<=lastmonF))
  if (y$Date[nrow(y)]<lastmonF) {
    ytmp <- ArimaForecast(y$VALUE,1)
    tmp1 <- y[nrow(y),]
    tmp1$Date <- y$Date[nrow(y)] %m+% months(1)
    tmp1$VALUE <- ytmp[length(ytmp)]
    y <- rbind(y,tmp1)
    return(y)
  }
}
...
PlotTempDisagg <- function(df1,df2,nam) {
  chrt <- ggplot()+
    ...
    labs(title=paste0("Chart for ",nam," = ",GDP[[nam]]$betterName),
...
MakeMonthlyP1Driver <- function(MODELSP_ChowLin,nam,Pause) {
  ...
  return(NewList <- list(MODELSP_ChowLin,round(result$VALUE,8)))
}
#------------------------------------------------------------------------------
# Function for tempdisagg with one driver series using Chow-Lin
# For GDP volume drivers
# Inputs: 
# var   - name of a column in the quarterly Vdata and 
#         monthly ChowLinInputdata data frames
# Pause - T or F to pause after chart
MakeMonthlyV2Driver <- function(MODELSP_ChowLin,nam,Pause) {
  df1 <- data.frame(Date=Vdata$Date,VALUE=Vdata[[nam]])
  df2 <- data.frame(Date=ChowLinInputdata$Date,VALUE=ChowLinInputdata[[nam]])
  model <- td(df1~1+df2,conversion="average",to="monthly",
    method="chow-lin-maxlog")
  result <- predict(model) 
  MODELSP_ChowLin[[length(MODELSP_ChowLin)+1]] <- summary(model)
  names(MODELSP_ChowLin)[length(MODELSP_ChowLin)] <- nam
  result <- rename(result,"Date"="time","VALUE"="value")
  result$VALUE <- round(result$VALUE,0)
  if (Pause) {
    PlotTempDisagg(df1,result,nam)
  }
  return(NewList <- list(MODELSP_ChowLin,round(result$VALUE,0)))
}
#------------------------------------------------------------------------------
# Function for tempdisagg with one driver series using Chow-Lin
# To re-benchmark the ByDeflation results
# Inputs: 
# var   - name of a column in the quarterly Vdata and 
#         monthly CDefldata data frames
# Pause - T or F to pause after chart
#MakeMonthlyVBenchDriver <- function(MODELSP_Bench,nam,Pause) {
#  df1 <- data.frame(Date=Vdata$Date,VALUE=Vdata[[nam]])
#  df2 <- data.frame(Date=Defldata$Date,VALUE=Defldata[[nam]])
#  lastMonthInQtr <- as.Date("2022-06-01")
#  df2 <- filter(df2,Date<=lastMonthInQtr)
#  model <- td(df1~1+df2,conversion="average",to="monthly",
#    method="chow-lin-maxlog")
#  result <- predict(model) 
#  MODELSP_Bench[[length(MODELSP_Bench)+1]] <- summary(model)
#  names(MODELSP_Bench)[length(MODELSP_Bench)] <- nam
#  result <- rename(result,"Date"="time","VALUE"="value")
#  result$VALUE <- round(result$VALUE,0)
#  if (Pause) {
#    PlotTempDisagg(df1,result,nam)
#  }
#  return(NewList <- list(MODELSP_Bench,round(result$VALUE,0)))
#}
#------------------------------------------------------------------------------
# Function for tempdisagg with two driver series using Chow-Lin
# For GDP volume drivers
...
#         monthly ChowLinInputdata data frames
# Pause - T or F to pause after chart
MakeMonthlyV22Driver <- function(MODELSP_ChowLin,nam,Pause) {
  df1 <- data.frame(Date=Vdata$Date,VALUE=Vdata[[nam]])
  var1 <- paste0(nam,"1")
  for (i in 1:ncol(ChowLinInputdata)) {if (colnames(ChowLinInputdata)[i]==var1) idx1 <- i}
  var2 <- paste0(nam,"2")
  for (i in 1:ncol(ChowLinInputdata)) {if (colnames(ChowLinInputdata)[i]==var2) idx2 <- i}
  df2 <- data.frame(Date=ChowLinInputdata$Date,VALUE=ChowLinInputdata[,idx1])
  df3 <- data.frame(Date=ChowLinInputdata$Date,VALUE=ChowLinInputdata[,idx2])
  model <- td(df1~1+df2+df3,conversion="average",to="monthly",
    method="chow-lin-maxlog")
  result <- predict(model) 
  MODELSP_ChowLin[[length(MODELSP_ChowLin)+1]] <- summary(model)
...
#------------------------------------------------------------------------------
# Function for tempdisagg with one driver series using Chow-Lin
# For GDP deflator drivers
# Inputs: 
# var   - name of a column in the quarterly Qdata and 
#         monthly CLdata data frames
# Pause - T or F to pause after chart
MakeMonthlyP2Driver <- function(MODELSP_ChowLin,nam,Pause) {
  df1 <- data.frame(Date=Pdata$Date,VALUE=Pdata[[nam]])
  df2 <- data.frame(Date=ChowLinInputdata$Date,VALUE=ChowLinInputdata[[nam]])
  model <- td(df1~1+df2,conversion="average",to="monthly",
    method="chow-lin-maxlog")
...
M_to_Q <- function(df) {
  ...
  df <- mutate(df,across(2:ncol(df),function(x) round(x,8)))
}
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
PC01 <- function(x) {y <- round(100*(x/lag(x)-1),8)}
#------------------------------------------------------------------------------
PC12 <- function(x) {y <- round(100*(x/lag(x,12)-1),8)}
